...
function setup_vendor() {
    ...
    export PIXELDUST_ROOT="$3"
    if [ ! -d "$PIXELDUST_ROOT" ]; then
        echo "\$PIXELDUST_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$PIXELDUST_ROOT/$OUTDIR" ]; then
        mkdir -p "$PIXELDUST_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$PIXELDUST_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$PIXELDUST_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$PIXELDUST_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$PIXELDUST_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        ...
        printf " * Copyright (C) 2019-$YEAR The PixelDust Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The PixelDust Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        if [ $INITIAL_COPYRIGHT_YEAR -lt 2016 ]; then
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-2016 The PixelDust Project\n" > $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq 2016 ]; then
            printf "# Copyright (C) 2016 The PixelDust Project\n" > $1
        fi
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The PixelDust Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The PixelDust Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The PixelDust Project\n" >> $1
        else
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$PIXELDUST_ROOT"/prebuilts/tools-custom/common/smali/baksmali.jar
        export SMALIJAR="$PIXELDUST_ROOT"/prebuilts/tools-custom/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$PIXELDUST_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$PIXELDUST_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$PIXELDUST_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$PIXELDUST_TARGET" >/dev/null; then
    ...
    for ARCH in $ARCHES; do
        ...
        elif [[ "$PIXELDUST_TARGET" =~ .jar$ ]]; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$PIXELDUST_ROOT"/"$OUTDIR"/proprietary
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$PIXELDUST_ROOT"/"$OUTDIR"/radio
